...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Process raw VTU outputs to extract output mesh."
    )
    parser.add_argument(
        "--input_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="File to save the canonical mesh."
    )
...
def main():
    ...
    recovered_points_displaced = unstitched_points[recover_indices]

    recovered_points_displaced[:, 1] -= recovered_points_displaced[:, 1].min()

    # print the bounding box of recovered_points_displaced
    print('------------bounding box stats---------------------')
    print('x:', recovered_points_displaced[:, 0].min(), recovered_points_displaced[:, 0].max())
    print('y:', recovered_points_displaced[:, 1].min(), recovered_points_displaced[:, 1].max())
    print('z:', recovered_points_displaced[:, 2].min(), recovered_points_displaced[:, 2].max())
    ...
    # load in trimesh and save again to clean up the mesh (remove duplicate vertices, etc.)
    mesh = trimesh.load(args.output_file, process=False)
